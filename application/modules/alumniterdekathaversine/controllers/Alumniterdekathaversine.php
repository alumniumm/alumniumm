...
Class Alumniterdekathaversine extends MX_Controller
{
	public function __construct()
	{
		...
		$this->load->model('Alumni_terdekat_haversine_model');
	...
	public function index()
	{
		...
		if($data['username']!=null)
		{
			...
			$config['center'] = '-3.677076, 119.647694';
          	...
          	$koordinatSemua = $this->Alumni_terdekat_haversine_model->ambil_lat_long_semua();	
          	foreach ($koordinatSemua as $koordinat) 
          	{
          		$id_alumni_fk = $koordinat->id_alumni_fk;
          		$tempatKerja = $koordinat->nama_instansi;
          		$dataInfo = $this->Alumni_model->ambil_data_alumni_per_id($id_alumni_fk);
		...
		if($data['username']!=null)
		{
			...
			if($tSubmit == 'Cari Alumni Terdekat')
			{
				...
				else
				{
		            ...
		            //proses haversine
		            $data['hasil_haversine'] = $this->Alumni_terdekat_haversine_model->haversine($latitude, $longitude);
