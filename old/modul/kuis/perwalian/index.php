...
if(empty($ss_user) or empty($ss_pwd)){
	...
	$title_page="Data Kuesioner Perwalian";
	...
	switch($act){
		default:
			...
			$sqld="SELECT * FROM $nama_table where bagian like \"%$gname%\" and status=\"1\" and kategori=\"2\" and hapus=\"0\" "."$selgbase order by urutan asc";
		...
		case"input":
			...
			if($tab=="tambah"){
				...
				$sqld="select * from $nama_table where hapus=\"0\" and kategori=\"2\" order by urutan desc limit 1";
				$data=mysql_query($sqld);
				$fdata=mysql_fetch_assoc($data);
		...
		case"proses":
			...
			if(empty($error)){
				if($edit=="edit"){
					...
					if($ndata>0){
						...
						if($fbagian==$bagian){
							...
							$sqld="select * from $nama_table where bagian=\"$bagian\" and hapus=\"0\"";
							$data=mysql_query($sqld);
							$ndata=mysql_num_rows($data);
							if($ndata==0){
				...
				if($edit=="tambah"){
					$sqld="select * from $nama_table where bagian=\"$bagian\" and kategori =\"2\" and hapus=\"0\"";
					$data=mysql_query($sqld);
					$ndata=mysql_num_rows($data);
					if($ndata==0){
						$vdata="bagian, urutan,kategori,tgl_insert, status, hapus";
						$vvalue="\"$bagian\", \"$urutan\", \"2\", \"$ndatetime\", \"$status\", \"0\"";
