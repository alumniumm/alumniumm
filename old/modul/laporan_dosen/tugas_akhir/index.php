...
if(empty($ss_user) or empty($ss_pwd)){
	...
	switch($act){
		...
		case"detail":
			...
			$gf2_kode_siswa=substr($gf2_kode_siswa,0,-1);
			if($gf2_kode_siswa!=""){
				$sqld="select * from $nama_table2 where no_dosen=\"$gid\"";
				$data=mysql($neomaa, $sqld);
				$ndata=mysql_num_rows($data);
				if($ndata>0){
						...
						while($fdata2=mysql_fetch_assoc($data2)){
							...
							if($ndata3>0){ $no=1; $jml1="";
								while($fdata3=mysql_fetch_assoc($data3)){
									...
									$sqld4="select * from $nama_table where id_pertanyaan=\"$idp\" and ".
											"bagian=\"$id_bagian\" and nim in ($gf2_kode_siswa) and ".
											"no_dosen=\"$gid\" group by nim, tahun_ajar, semester order by nim asc";
									$data4=mysql_query($sqld4);
									$jml_data=mysql_num_rows($data4);
									if($jml_data>0){
										while($fdata4=mysql_fetch_assoc($data4)){
											$fnim=$fdata4["nim"];
											$fthn1=$fdata4["tahun_ajar"];
											$fsmt1=$fdata4["semester"];
											$nilai_max2="";
											
											$sqld5="select * from $nama_table where id_pertanyaan=\"$idp\" and ".
													"bagian=\"$id_bagian\" and nim in ($fnim) and ".
													"no_dosen=\"$gid\" and tahun_ajar=\"$fthn1\" and ".
													"semester=\"$fsmt1\"";
											$data5=mysql_query($sqld5);
											$ndata5=mysql_num_rows($data5);
											if($ndata5>0){
												while($fdata5=mysql_fetch_assoc($data5)){
													$isi_kurang_s=$fdata5["isi_kurang_s"];
													$isi_kurang=$fdata5["isi_kurang"];
													$isi_baik=$fdata5["isi_baik"];
													$isi_baik_s=$fdata5["isi_baik_s"];
													...
													$nilai_max1=@max($isi_kurang_s, $isi_kurang, $isi_baik, $isi_baik_s);
													$nilai_max2=$nilai_max2 + $nilai_max1;
												}
											}else{
												$nilai_max2="";
											}
											
											$nilai_max=$nilai_max + $nilai_max2;
										}
									}else{
										$nilai_max="";
									}
