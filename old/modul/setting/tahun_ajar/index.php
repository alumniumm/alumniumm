...
if(empty($ss_user) or empty($ss_pwd)){
	...
	$title_page="Data Tahun Akademik";
	...
	switch($act){
		default:
			...
			$sqld="SELECT * FROM $nama_table where tahun_ajar like \"%$gname%\" and status=\"0\" ".
				...
				$cnpage="<form name=\"form1\" action=\"?pages=$pages&act=proses\" method=\"post\">
					<table class='pad5'>
						<tr>
							...
							<td>:</td>
							<td>
								<input type=\"text\" name=\"xthn1\" value=\"$xthn1\" size=\"2\" maxlength='4' />/
								<input type=\"text\" name=\"xthn2\" value=\"$xthn2\" size=\"2\" maxlength='4' />
							</td>
						</tr>
					</table><br />
					$edit
		...
		case"proses":
			...
			if($xthn1>=$xthn2 or $xthn2!=$ptahun){
			...
			if(empty($error)){
				if($edit=="edit"){
					...
					if($ndata>0){
						$fdata=mysql_fetch_assoc($data);
						$tahun_ajar=$fdata["tahun_ajar"];
						
						if($tahun_ajar==$gtahun){
							$cnpage="<h4 class=\"alert_success\">Data Berhasil Dirubah</h4>";
						}else{
							$sqld="select * from $nama_table where tahun_ajar=\"$gtahun\" and status=\"0\"";
							$data=mysql_query($sqld);
							$ndata=mysql_num_rows($data);
							if($ndata==0){
								$vdata="tahun_ajar=\"$gtahun\", tgl_update=\"$ndatetime\"";
								$vvalue="id=\"$pid\"";
				...
				if($edit=="tambah"){
					...
					$ndata=mysql_num_rows($data);
					if($ndata==0){
						$vdata="tahun_ajar, tgl_insert, status";
						$vvalue="\"$gtahun\", \"$ndatetime\", \"0\"";
